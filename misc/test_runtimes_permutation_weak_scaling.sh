...
for j in {1..3}; do
   for num_executors in 1 2 4 8 ; do

	if [ $instance_scaling = true ]; then  
    	  echo -e "\n\n===============\nINSTANCE SCALING\n\n"
	  #instance scaling
	  inst_scaling=$((3500*num_executors))
	  start=`date '+%F_%H:%M:%S'`
	...
	if [ $layer_scaling = true ]; then
	  ...
	  echo -n "layer_scaling_adult_fnn,${num_executors},${instances},107,${num_executors},${start}," | tee -a "$data_file"
	  runtime_layers=$(runspark-num-executors ${num_executors} -f /home/lepage/scripts/shap/examples/shapley-permutation-experiment-spark.dml -stats 1 -nvargs remove_non_var=0 use_partitions=0 n_permutations=${permutations} integration_samples=${samples} rows_to_explain=${instances} write_to_file=0 execution_policy=by-row data_dir=/user/hadoop/shap/adult/ X_bg_path=Adult_X.csv B_path=ffn_${num_executors}l_model.bin metadata_path=Adult_partitions.csv model_type=ffPredict_${num_executors}l)
	  end=`date '+%F_%H:%M:%S'`
